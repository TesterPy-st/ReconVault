...
from app.models import Target, TargetStatus, TargetType
from app.models.audit import AuditAction, AuditLog, AuditSeverity
from app.schemas.target import (TargetBulkRequest, TargetBulkResponse,
                                TargetCreate, TargetListResponse,
                                TargetResponse, TargetSearchRequest,
                                TargetSearchResponse, TargetStats,
                                TargetUpdate)
...
class TargetService:
    ...
    def create_target(
        self, target_data: TargetCreate, user_id: Optional[int] = None
    ) -> TargetResponse:
        """
        Create a new target.
    ...
    def get_targets(
        ...
        try:
            ...
            total = query.count()
            targets = (
                query.order_by(desc(Target.created_at)).offset(skip).limit(limit).all()
            )
    ...
    def update_target(
        self, target_id: int, target_data: TargetUpdate, user_id: Optional[int] = None
    ...
    def search_targets(
        self, search_request: TargetSearchRequest
        ...
        try:
            ...
            total = query.count()
            targets = (
                query.order_by(desc(Target.risk_score))
    ...
    def get_target_statistics(self) -> TargetStats:
        ...
        try:
            ...
            active_targets = (
                self.db.query(Target).filter(Target.is_active == True).count()
            )
            ...
            for target_type in TargetType:
                count = (
                    ...
                    .filter(and_(Target.type == target_type, Target.is_active == True))
                    .count()
                )
            ...
            for status in TargetStatus:
                count = (
                    self.db.query(Target)
                    .filter(and_(Target.status == status, Target.is_active == True))
                    .count()
                )
            ...
            risk_counts = {
                "critical": self.db.query(Target)
                .filter(and_(Target.risk_score >= 0.8, Target.is_active == True))
                .count(),
                "high": self.db.query(Target)
                .filter(
                    and_(
                        Target.risk_score >= 0.6,
                        Target.risk_score < 0.8,
                        Target.is_active == True,
                    )
                )
                .count(),
                "medium": self.db.query(Target)
                .filter(
                    and_(
                        Target.risk_score >= 0.4,
                        Target.risk_score < 0.6,
                        Target.is_active == True,
                    )
                )
                .count(),
                "low": self.db.query(Target)
                .filter(
                    and_(
                        Target.risk_score >= 0.2,
                        Target.risk_score < 0.4,
                        Target.is_active == True,
                    )
                )
                .count(),
                "minimal": self.db.query(Target)
                .filter(and_(Target.risk_score < 0.2, Target.is_active == True))
                .count(),
            }

            # Calculate average risk score
            avg_risk = (
                self.db.query(Target.risk_score).filter(Target.is_active == True).all()
            )
            average_risk_score = (
    ...
    def bulk_create_targets(
        self, bulk_request: TargetBulkRequest, user_id: Optional[int] = None
    ) -> TargetBulkResponse:
        """
        ...
        try:
            ...
            for i, target_data in enumerate(bulk_request.targets):
                try:
                    ...
                    self.db.rollback()
                    failed.append(
                        {"index": i, "reason": str(e), "value": target_data.value}
                    )
