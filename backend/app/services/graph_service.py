...
from app.intelligence_graph.graph_models import (EntityNode, GraphData,
                                                 GraphEdge, GraphNode,
                                                 IntelligenceNode, NodeType,
...
class GraphService:
            ...
            if neo4j_healthy:
                try:
                    ...
                    performance_metrics = {
                        ...
                        "total_relationships": stats.get("relationship_count", {}).get(
                            "count", 0
                        ),
        ...
        try:
            ...
            if "label_counts" in stats_data:
                ...
                for label_info in label_counts:
                    node_types[label_info.get("label", "Unknown")] = label_info.get(
                        "count", 0
                    )
    ...
    def create_node(self, node_data: GraphNodeCreate) -> GraphNodeCreateResponse:
        ...
        try:
            ...
            created_node = self.graph_ops.create_node(
                node_type=node_data.label, properties=node_data.properties
            )
            ...
            return GraphNodeCreateResponse(
                success=False, node_id=None, node=None, message=str(e)
            )
    ...
    def create_edge(self, edge_data: GraphEdgeCreate) -> GraphEdgeCreateResponse:
        ...
        try:
            ...
            if created_relationship:
                ...
                return GraphEdgeCreateResponse(
                    success=False, edge=None, message="Failed to create relationship"
                )

        except Exception as e:
            logger.error(f"Edge creation failed: {e}")
            return GraphEdgeCreateResponse(success=False, edge=None, message=str(e))

    def update_node(
        self, node_id: int, update_data: GraphNodeUpdate
        ...
        try:
            success = self.graph_ops.update_node(
                node_id=node_id, properties=update_data.properties
            )
            ...
            return GraphNodeUpdateResponse(
                ...
                if success
                else "Failed to update node",
            )

        except Exception as e:
            logger.error(f"Node update failed: {e}")
            return GraphNodeUpdateResponse(
                success=False, node_id=node_id, message=str(e)
            )
    ...
    def export_graph(self, export_request: GraphExportRequest) -> GraphExportResponse:
        ...
        try:
            ...
            export_data = self.graph_ops.export_graph_data(
    ...
    def analyze_graph(
        self, analysis_request: GraphAnalysisRequest
