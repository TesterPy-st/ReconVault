...
from app.schemas.entity import (
    ...
    EntityBulkRequest, EntityBulkResponse
)
...
class EntityService:
    ...
    def create_entity(self, entity_data: EntityCreate, user_id: Optional[int] = None) -> EntityResponse:
        """
        Create a new entity.
        ...
        try:
            ...
            if entity_data.target_id:
                target = self.db.query(Target).filter(
                    and_(Target.id == entity_data.target_id, Target.is_active == True)
                ).first()
                if not target:
                    raise ValueError(f"Target {entity_data.target_id} not found or inactive")
    ...
    def get_entities(
        ...
        try:
            ...
            total = query.count()
            entities = query.order_by(desc(Entity.risk_score)).offset(skip).limit(limit).all()
    ...
    def search_entities(self, search_request: EntitySearchRequest) -> EntitySearchResponse:
        """
        ...
        try:
            ...
            total = query.count()
            entities = query.order_by(desc(Entity.risk_score)).offset(search_request.offset).limit(search_request.limit).all()
    ...
    def verify_entity(self, entity_id: int, verified: bool = True, user_id: Optional[int] = None) -> bool:
        """
        ...
        try:
            ...
            # Log audit event
            action = AuditAction.ENTITY_VERIFY if verified else AuditAction.ENTITY_UPDATE
            self._log_audit_event(
                action,
                user_id,
            ...
            logger.info(f"{'Verified' if verified else 'Unverified'} entity {entity_id}")
            return True
    ...
    def add_tags(self, entity_id: int, tags: List[str], user_id: Optional[int] = None) -> bool:
        """
    ...
    def remove_tags(self, entity_id: int, tags: List[str], user_id: Optional[int] = None) -> bool:
        """
    ...
    def get_entity_statistics(self) -> EntityStats:
        ...
        try:
            ...
            active_entities = self.db.query(Entity).filter(Entity.is_active == True).count()
            verified_entities = self.db.query(Entity).filter(
                and_(Entity.is_verified == True, Entity.is_active == True)
            ).count()
            entities_with_target = self.db.query(Entity).filter(
                and_(Entity.target_id.isnot(None), Entity.is_active == True)
            ).count()
            ...
            for entity_type in EntityType:
                count = self.db.query(Entity).filter(
                    and_(Entity.type == entity_type, Entity.is_active == True)
                ).count()
                type_counts[entity_type] = count
            
            # Get source distribution
            source_counts = {}
            sources = self.db.query(Entity.source).filter(Entity.is_active == True).distinct().all()
            ...
            risk_counts = {
                "critical": self.db.query(Entity).filter(
                    and_(Entity.risk_score >= 0.8, Entity.is_active == True)
                ).count(),
                "high": self.db.query(Entity).filter(
                    and_(Entity.risk_score >= 0.6, Entity.risk_score < 0.8, Entity.is_active == True)
                ).count(),
                "medium": self.db.query(Entity).filter(
                    and_(Entity.risk_score >= 0.4, Entity.risk_score < 0.6, Entity.is_active == True)
                ).count(),
                "low": self.db.query(Entity).filter(
                    and_(Entity.risk_score >= 0.2, Entity.risk_score < 0.4, Entity.is_active == True)
                ).count(),
                "minimal": self.db.query(Entity).filter(
                    and_(Entity.risk_score < 0.2, Entity.is_active == True)
                ).count()
            }
            
            # Calculate average risk score
            avg_risk = self.db.query(Entity.risk_score).filter(Entity.is_active == True).all()
