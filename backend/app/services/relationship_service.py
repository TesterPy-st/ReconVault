    ...
    def create_relationship(
        ...
        try:
            ...
            if source_entity_id == target_entity_id:
                raise InvalidRelationshipError("Cannot create self-referential relationship")
    ...
    def _sync_to_neo4j(self, relationship: Relationship) -> None:
        ...
        try:
            ...
            if not source_entity or not target_entity:
                logger.warning(f"Cannot sync relationship {relationship.id}: entities not found")
                return
            
            # Create relationship in Neo4j
            self.neo4j_client.create_relationship(
                start_node_label="Entity",
                start_node_properties={"id": str(source_entity.id), "value": source_entity.value},
                relationship_type=relationship.type.upper().replace("-", "_"),
                end_node_label="Entity",
                end_node_properties={"id": str(target_entity.id), "value": target_entity.value},
