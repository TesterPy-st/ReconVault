...
class User(Base):
    ...
    status = Column(String(30), default=UserStatus.PENDING_VERIFICATION, nullable=False, index=True)
...
User.__table__.append_column(
    Column("idx_user_username_email", String(150), index=True)
)

User.__table__.append_column(
    Column("idx_user_role_status", String(50), index=True)
)