...
from app.schemas.target import (TargetBulkRequest, TargetBulkResponse,
                                TargetCreate, TargetDeleteResponse,
                                TargetListResponse, TargetResponse,
                                TargetSearchRequest, TargetSearchResponse,
                                TargetStats, TargetUpdate)
...
async def get_targets(
    ...
    limit: int = Query(
        100, ge=1, le=1000, description="Maximum number of targets to return"
    ),
...
async def update_target(
    target_id: int, target_data: TargetUpdate, db: Session = Depends(get_db)
):
    """
...
async def search_targets(
    search_request: TargetSearchRequest, db: Session = Depends(get_db)
):
    """
...
async def get_target_entities(
    ...
    try:
        ...
        entities_response = entity_service.get_entities(
            skip=skip, limit=limit, active_only=True, target_id=target_id
        )
...
async def get_target_risk_assessment(target_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        entity_service = get_entity_service(db)
        entities_response = entity_service.get_entities(
            limit=1000, active_only=True, target_id=target_id
        )
        ...
        risk_assessment = {
            ...
            "high_risk_entities": len(
                [e for e in entities_response.entities if e.risk_score >= 0.7]
            ),
