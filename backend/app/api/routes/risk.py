...
from app.schemas.risk import (RiskAssessmentListResponse,
                              RiskAssessmentResponse, RiskAssessRequest,
                              RiskAssessResponse, RiskSummary)
...
def get_entity_risk_assessment(entity_id: UUID, db: Session = Depends(get_database)):
    """
...
def trigger_risk_assessment(
    request: RiskAssessRequest, db: Session = Depends(get_database)
):
    """
    ...
    try:
        ...
        logger.info(
            f"Triggered risk assessment job: {job_id} for {entity_count} entities"
        )
...
def get_risk_summary(db: Session = Depends(get_database)):
    """
    ...
    try:
        ...
        critical_count = (
            db.query(RiskAssessment).filter(RiskAssessment.level == "critical").count()
        )
        high_count = (
            db.query(RiskAssessment).filter(RiskAssessment.level == "high").count()
        )
        medium_count = (
            db.query(RiskAssessment).filter(RiskAssessment.level == "medium").count()
        )
        low_count = (
            db.query(RiskAssessment).filter(RiskAssessment.level == "low").count()
        )
        info_count = (
            db.query(RiskAssessment).filter(RiskAssessment.level == "info").count()
        )
