...
from app.schemas.graph import (GraphAnalysisRequest, GraphAnalysisResponse,
                               ...
                               GraphPathResponse, GraphRecommendationsResponse,
...
async def search_graph(search_request: GraphSearchRequest):
    """
...
async def expand_graph(expand_request: GraphExpandRequest):
    """
    ...
    try:
        ...
        logger.info(
            f"Graph expansion completed: {results.expanded_graph.total_nodes} nodes"
        )
        return results
...
async def find_path(path_request: GraphPathRequest):
    """
...
async def create_graph_node(node_data: GraphNodeCreate):
    """
...
async def create_graph_edge(edge_data: GraphEdgeCreate):
    """
...
async def update_graph_node(node_id: int, update_data: GraphNodeUpdate):
    """
...
async def export_graph(
    export_request: GraphExportRequest, background_tasks: BackgroundTasks = None
):
    """
    Export graph data.
...
async def analyze_graph(
    analysis_request: GraphAnalysisRequest, background_tasks: BackgroundTasks = None
):
    """
...
async def build_graph_from_database(background_tasks: BackgroundTasks = None):
    """
    ...
    try:
        ...
        result = {
            ...
            "message": "Graph built from database successfully"
            if success
            else "Graph building failed",
        }
...
async def sync_graph_with_database():
    ...
    try:
        ...
        result = {
            "status": "completed" if success else "failed",
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "message": "Graph synchronized with database successfully"
            if success
            else "Graph synchronization failed",
        }
...
async def initialize_graph_service():
    ...
    try:
        ...
        result = {
            ...
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "message": "Graph service initialized successfully"
            if success
            else "Graph service initialization failed",
        }
