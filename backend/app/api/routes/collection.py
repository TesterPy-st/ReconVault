...
from app.schemas.osint import (CollectionStats, CollectionTaskCreate,
...
@router.post(
    "/start", response_model=CollectionTaskResponse, status_code=status.HTTP_201_CREATED
)
def start_collection_task(
    task_data: CollectionTaskCreate, db: Session = Depends(get_database)
):
    """
...
def get_collection_task(task_id: UUID, db: Session = Depends(get_database)):
    """
...
def get_collection_results(task_id: UUID, db: Session = Depends(get_database)):
    """
