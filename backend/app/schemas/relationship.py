...
class RelationshipWithEntities(RelationshipResponse):
    ...
    source_entity: Optional[Dict[str, Any]] = Field(
        None, description="Source entity data"
    )
    target_entity: Optional[Dict[str, Any]] = Field(
        None, description="Target entity data"
    )
...
class RelationshipSearchRequest(BaseModel):
    ...
    type: Optional[RelationType] = Field(
...
class RelationshipBulkCreate(BaseModel):
    ...
    relationships: list[RelationshipCreate] = Field(
        ..., min_items=1, max_items=100, description="Relationships to create"
    )
...
class RelationshipBulkResponse(BaseModel):
    ...
    created: list[RelationshipResponse] = Field(
        ..., description="Successfully created relationships"
    )
    skipped: list[Dict[str, Any]] = Field(
        default_factory=list, description="Skipped relationships"
    )
    failed: list[Dict[str, Any]] = Field(
        default_factory=list, description="Failed relationships"
    )
