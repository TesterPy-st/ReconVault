...
from app.models import TargetType, TargetStatus
...
class TargetUpdate(BaseModel):
    ...
    value: Optional[str] = Field(None, min_length=1, max_length=500, description="Target value")
...
class TargetStats(BaseModel):
    ...
    targets_by_status: Dict[str, int] = Field(..., description="Targets grouped by status")
    targets_by_risk_level: Dict[str, int] = Field(..., description="Targets grouped by risk level")
...
class TargetBulkRequest(BaseModel):
    ...
    targets: list[TargetCreate] = Field(..., min_items=1, max_items=100, description="Targets to create")
...
class TargetBulkResponse(BaseModel):
    ...
    created: list[TargetResponse] = Field(..., description="Successfully created targets")
    skipped: list[Dict[str, Any]] = Field(default_factory=list, description="Skipped targets")
    failed: list[Dict[str, Any]] = Field(default_factory=list, description="Failed targets")
...
class TargetImportRequest(BaseModel):
    ...
    update_existing: bool = Field(False, description="Whether to update existing targets")
