...
class RiskAssessmentBase(BaseModel):
    ...
    factors: List[str] = Field(default_factory=list, description="Risk factors contributing to score")
...
class RiskAssessmentUpdate(BaseModel):
    ...
    score: Optional[float] = Field(None, ge=0.0, le=1.0, description="Numeric risk score (0-1)")
...
class RiskAssessmentListResponse(BaseModel):
    ...
    assessments: list[RiskAssessmentResponse] = Field(..., description="List of risk assessments")
...
class RiskSummary(BaseModel):
    ...
    average_risk_score: float = Field(..., description="Average risk score across all entities")
    highest_risk_entities: List[Dict[str, Any]] = Field(default_factory=list, description="Top risk entities")
...
class RiskSearchRequest(BaseModel):
    ...
    min_score: Optional[float] = Field(None, ge=0.0, le=1.0, description="Minimum risk score")
    max_score: Optional[float] = Field(None, ge=0.0, le=1.0, description="Maximum risk score")
    factors: Optional[List[str]] = Field(None, description="Filter by risk factors")
    assessed_after: Optional[datetime] = Field(None, description="Filter by assessment date")
    assessed_before: Optional[datetime] = Field(None, description="Filter by assessment date")
...
class RiskAssessRequest(BaseModel):
    ...
    entity_ids: Optional[List[UUID]] = Field(None, description="Specific entity IDs to assess")
    assess_all: bool = Field(False, description="Assess all entities")
    force_reassess: bool = Field(False, description="Force reassessment even if recent assessment exists")
...
class RiskAssessResponse(BaseModel):
    ...
    entities_queued: int = Field(..., description="Number of entities queued for assessment")
