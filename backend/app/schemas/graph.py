...
class GraphExpandRequest(BaseModel):
    ...
    relationship_types: Optional[List[str]] = Field(None, description="Relationship types to include")
...
class GraphStatistics(BaseModel):
    ...
    relationship_types: Dict[str, int] = Field(..., description="Relationship counts by type")
...
class GraphEdgeCreate(BaseModel):
    ...
    def validate_relationship_type(cls, v):
        ...
        if v not in valid_types:
            raise ValueError(f"Invalid relationship type: {v}. Valid types: {valid_types}")
        return v
...
class GraphNodeDeleteResponse(BaseModel):
    ...
    relationships_deleted: int = Field(..., description="Number of relationships deleted")
...
class GraphRecommendation(BaseModel):
    ...
    nodes_affected: Optional[List[int]] = Field(None, description="Node IDs that would be affected")


class GraphRecommendationsResponse(BaseModel):
    """Schema for graph recommendations responses"""
    
    recommendations: List[GraphRecommendation] = Field(..., description="List of recommendations")
